...
export default function Index() {
    const { packages, flash = {} } = usePage().props;

    const packagesItems = packages.data || [];
    const { delete: deletedata } = useForm();

    const handleDelete = (id) => {
        if (window.confirm("Do you want to Delete?")) {
            deletedata(route("package.destroy", { package: id }));
        }
    };
    return (
        <Layout>
            <Head title="All Package" ></Head>
            <div className="row">
                <h1 className="p-4 text-center h1">All Packages</h1>
